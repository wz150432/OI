...
exKMP 可以线性的求出 $\forall x \in [0, n]$ ，$[0, n]$ 与 $[x, n]$ 的最长公共前缀的长度  $ne_i$。 首先我们先假设我们已经知道了 $0\sim x - 1$ 的 $ne$ 函数的值。我们让其中长度最长的下标为 $k$，根据定义我们很容易发现 $[0, ne_k - 1]$ 等于 $[k, k + ne_k - 1]$ ，我们让 $p = k + ne_k - 1$ ，$[x - k, ne_k - 1]$ 等于 $[x, p]$ ，因为他是前者的子集。我们让 $l = ne_{x-k}$，如果 $l > p-x$ 我们暴力扫，其他情况 $ne_k=l$。
