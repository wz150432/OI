...
我们很容易发现我们匹配长度每次最多增加一，也就是当我们匹配的时候如果 $s_i \neq s_{ne_{i-1}+1}$ 我们直接跳到匹配 $s_{ne_{ne_{i-1}} +1}$。为什么这之间的区间都不可能呢？很简单，根据定义我们知道区间 $1 \sim ne_{i - 1}$ 和 $i - ne_{i-1} \sim i - 1$ 是等价的，所以这个后缀的区间也同样可以使用前面计算出的 $ne$ 函数，如果这时两个区间内还有值使得 $ne_i$ 更大，说明我们之前算的 $ne_i$ 不是最大显然矛盾。
