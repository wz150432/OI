...
- **WIDTH**：定义向量中每个元素存储几位数字。比如`WIDTH = 1`时，每个元素存 1 位（$0\sim 9$）；`WIDTH = 4`时，每个元素存 4 位（$0\sim9999$），此时 `BASE` 需设为 `10000`。
- **BASE**：等于`10^WIDTH`，是每段数字的 “进位阈值”。比如`WIDTH = 1`时`BASE = 10`，符合十进制的进位规则。

**为什么要分段？** 当数字极长时（比如 1e4 位），`WIDTH = 4`能让向量长度从 1e4 缩减到 2500，减少循环次数，提升效率。本文代码用`WIDTH =  1`是为了简化理解，原理可直接迁移到更大的`WIDTH`。
...
**关键设计：逆序存储**
手工计算时，加减乘除都从**最低位（个位）** 开始，逆序存储能让最低位位于向量的起始位置（`a[0]`），无需反转数据就能直接循环处理，极大简化代码。

例如：数字`123`在向量中存储为 `a = [3, 2, 1]`（`a[0]=3`个位，`a[1]=2`十位，`a[2]=1`百位）。
...
- `extra =  4 % 1  =  0`，循环从`i = 3`（字符 '4'）开始，每次取 1 位，依次 push 4、3、2、1，向量最终为`[4,3,2,1]`。

#### （3）输出函数：逆序向量转正常数字

```cpp
void print() const {
    // 向量最后一个元素是最高位（如a=[4,3,2,1]，最高位是1）
    printf("%d", a.back());
...
示例：向量`[4,3,2,1]`输出时，先打印`a.back() = 1`，再依次打印`2`、`3`、`4`，最终输出`1234`。
...
// 小于：先比长度（短的小），长度相同则从最高位（向量末尾）往低位比
bool operator<(const number& x) const {
...
3. 新进位 = 总和 / BASE（取商进位）；
4. 处理最后一位的进位（若有），并删除向量末尾的无效 0（如结果为`100`，向量存`[0,0,1]`，无需保留多余的 0）。
...
3. 若结果非负，借位设为 0；
4. 删除向量末尾的无效 0（如结果为`0`，向量存`[0]`，避免存`[0,0]`）。
...
number operator-(const number& x) const {
    ...
    // 删除末尾的无效0（如结果为100-50=50，向量为[0,5]，无需保留前面的0）
...
number operator*(const number& x) const {
    ...
    // 删除末尾的无效0（如20*30=600，向量为[0,0,6]，无需保留前面的0）
    while (res.a.size() > 1 && !res.a.back())
        res.a.pop_back();
    return res;
}
...
1. **调整 WIDTH 和 BASE**：将`WIDTH = 4`，`BASE = 10000`，减少向量长度，提升运算速度；
